...
struct rxs_encoder_config {
  uint32_t width;                       /* height of the image buffer you pass into rxs_encoder_encode, as I420 (or any other supported format) */
  uint32_t height;                      /* height of the image buffer you pass into rxs_encoder_encode, as I420 (or any other supported format) */
  int fps_num;                          /* FPS numerator, e.g. 1 */
  int fps_den;                          /* FPS denumerator, e.g. 25 for 25fps */
  int fmt;                              /* the pixel format of the pixels you pass into rxs_encoder_encode, e.g VPX_IMG_FMT_I420. */
};
...
struct rxs_encoder {
  ...
  int fps_num;
  int fps_den;
  int fmt;                              /* pixel format that is used to pass data into rxs_encoder_encode().. */
...
int rxs_encoder_encode(rxs_encoder* enc, unsigned char* pixels, int64_t pts);           /* encode the given buffer */
